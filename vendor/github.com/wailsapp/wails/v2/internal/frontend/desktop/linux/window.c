...
GtkWidget *SetupWebview(void *contentManager, GtkWindow *window, int hideWindowOnClose, int gpuPolicy, int disableWebViewDragAndDrop, int enableDragAndDrop)
{
    ...
    const char *prop_names[] = {"user-content-manager", "settings", "website-policies"};
    GValue prop_values[] = {G_VALUE_INIT, G_VALUE_INIT, G_VALUE_INIT};
    g_value_init(&prop_values[0], G_TYPE_OBJECT);
    g_value_set_object(&prop_values[0], contentManager);
    g_value_init(&prop_values[1], G_TYPE_OBJECT);
    g_value_set_object(&prop_values[1], settings);
    g_value_init(&prop_values[2], G_TYPE_OBJECT);
    g_value_set_object(&prop_values[2], policies);

    GtkWidget *webview = GTK_WIDGET(g_object_new_with_properties(WEBKIT_TYPE_WEB_VIEW, 3, prop_names, prop_values));

    g_value_unset(&prop_values[0]);
    g_value_unset(&prop_values[1]);
    g_value_unset(&prop_values[2]);
